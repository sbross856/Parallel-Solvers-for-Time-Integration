...
def NavierStokes2D(T, IC, IC1, N, L, M):
    ...
    def func(w, psi):
        ...
        for i in range(1, L-1):
            ...
            f[0, i, 0] = -((psi[0, i + 1] - psi[0, i - 1]) * (zeta[1, i] - zeta[-2, i])) / (4 * dx ** 2) + \
                        ((psi[1, i] - psi[-1, i]) * (zeta[0, i + 1] - zeta[0, i - 1])) / (4 * dx ** 2) + \
                         visc * (zeta[1, i] + zeta[-1, i] + zeta[0, i + 1] + zeta[0, i - 1] - 4 * zeta[0, i]) / dx ** 2

            f[i, 0, 0] = -((psi[i, 1] - psi[i, -2]) * (zeta[i + 1, 0] - zeta[i - 1, 0])) / (4 * dx ** 2) + \
                         ((psi[i + 1, 0] - psi[i - 1, 0]) * (zeta[i, 1] - zeta[i, -1])) / (4 * dx ** 2) + \
                         visc * (zeta[i + 1, 0] + zeta[i - 1, 0] + zeta[i, 1] + zeta[i, -1] - 4 * zeta[i, 0]) / dx ** 2

        # updating corner
        f[0, 0, 0] = -((psi[0, 1] - psi[0, -1]) * (zeta[1, 0] - zeta[-1, 0])) / (4 * dx ** 2) + \
                     ((psi[1, 0] - psi[-1, 0]) * (zeta[0, 1] - zeta[0, -1])) / (4 * dx ** 2) + \
                     visc * (zeta[-1, 0] + zeta[1, 0] + zeta[0, -1] + zeta[0, 1] - 4 * zeta[0, 0]) / dx ** 2
        return f

    # Solving the poisson equation for the stream function
    ...
    for iTime in range(0, M - 1):
        ...
        stream[iTime + 1, :, :] = poisson(Y2[0, :, :, 0], stream[iTime, :, :])
    ...
    for iCor in range(1, M - 1):
        ...
        for iTime in range(0, M - 1):
            ...
            stream[iTime + 1, :, :] = poisson(Y2[iCor, :, :, 0], stream[iTime, :, :])
    ...
    for iTime in range(0, M - 1):
        ...
        stream[iTime + 1, :, :] = poisson(Y2[M - 1, :, :, 0], stream[iTime, :, :])
        yy[iTime + 1, :, :, :] = Y2[M - 1, :, :, :]
    ...
    for iTime in range(M - 1, 2 * M - 2):
        ...
        stream[iTime + 1, :, :] = poisson(Y2[M - 1, :, :, 0], stream[iTime, :, :])  # updating stream function array
    ...
    for iTime in range(2 * M - 2, N + M - 1):
        ...
        stream[iTime + 1 - (M - 1), :, :] = poisson(Y2[M - 1, :, :, 0], stream[iTime - (M - 1), :, :])
...
# Initial conditions for the taylor green problem
#  Vorticity
def ICfunc1(x, y):
...
zeta, psi = NavierStokes2D(1, ICfunc1, ICfunc2, 1000, 128, 4)
